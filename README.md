...
This repository includes a demo MCP server implemented with the official TypeScript SDK that highlights different widget bundles. Every tool response includes plain text content, structured JSON, and `_meta.openai/outputTemplate` metadata so the Apps SDK can hydrate the matching widget.
